...
class ShopifyClient {
	...
	public function call($method, $path, $params=array(), $version = '2020-01')
	{
		$baseurl = "https://{$this->shop_domain}/";
		$path = str_replace( '/admin/', '/admin/api/' . $version . '/', $path );
		...
		//cursor based pagination, find next/previous page info param
		if( isset( $this->last_response_headers['link'] ) ) {
			$br = explode( ',', $this->last_response_headers['link'] );
			foreach( $br as $brs ) {
				$bbr = explode( ';', $brs );
				if( strpos( $bbr[1], 'next' ) !== false ) {
					$links['next_page_params'] = explode( '?', str_replace( array('<', '>'), '', $bbr[0] ) )[1];
				} elseif( strpos( $bbr[1], 'previous' ) !== false ) {
					$links['previous_page_params'] = explode( '?', str_replace( array('<', '>'), '', $bbr[0] ) )[1];
				}
			}
		}
