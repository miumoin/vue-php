...
class SofedaClient {
	...
	public function __construct($shop_domain, $token, $apibaseurl, $api_key, $secret) {
		$this->apibaseurl = $apibaseurl;
		$this->name = "SofedaClient";
	...
	public function call($method, $path, $params=array())
	{
		...
		// add auth headers
		$request_headers[] = 'X-Sofeda-Shop-Domain: ' . $this->shop_domain;
		$request_headers[] = 'X-Sofeda-Access-Token: ' . $this->token;
		$request_headers[] = 'X-Sofeda-Method-Called: ' . $method;

		$response = $this->curlHttpApiRequest($method, $url, $query, $payload, $request_headers);
		$response = json_decode($response, true);

		if (isset($response['errors']) or ($this->last_response_headers['http_status_code'] >= 400))
			throw new SofedaApiException($method, $path, $params, $this->last_response_headers, $response);
	...
	private function curlHttpApiRequest($method, $url, $query='', $payload='', $request_headers=array())
	{
		...
		if ($errno) throw new SofedaCurlException($error, $errno);
	...
	private function curlSetopts($ch, $method, $payload, $request_headers)
	{
		...
		curl_setopt($ch, CURLOPT_USERAGENT, 'sofeda-php-api-client');
	...
	private function shopApiCallLimitParam($index)
	{
		...
		$params = explode('/', $this->last_response_headers['http_x_sofeda_shop_api_call_limit']);
		return (int) $params[$index];
	}	
}

class SofedaCurlException extends Exception { }
class SofedaApiException extends Exception
{
	protected $method;
	protected $path;
	protected $params;
